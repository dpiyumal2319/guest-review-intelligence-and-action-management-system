...
DEFAULT_SOURCE_CODE = "qwen_synthetic_draft"
DEFAULT_NOTES = "qwen-generated draft; requires human review"
PROMPT_VERSION = "qwen-issue-label-drafts-v1"
...
def build_prompt(*, category_code: str, category_name: str, category_description: str, count: int) -> str:
    return (
        ...
        "Rules:\n"
        "- Each text must be one realistic English hotel guest review sentence or short paragraph.\n"
        "- Text must clearly match only the requested category.\n"
...
def generate_label_drafts(
    *,
    ...
    for category_index, category in enumerate(ISSUE_CATEGORIES, start=1):
        ...
        attempts = 0
        expected_batches = (rows_per_category + batch_size - 1) // batch_size
        max_attempts = expected_batches + max_retries
        if log is not None:
            log(
                f"[{category_index}/{len(ISSUE_CATEGORIES)}] generating {rows_per_category} "
                f"rows for {category_code}"
            )
        while len(category_rows) < rows_per_category and attempts < max_attempts:
            attempts += 1
            target_for_batch = min(batch_size, rows_per_category - len(category_rows))
            prompt = build_prompt(
            ...
            parsed_rows = parse_generated_rows(requester(prompt), category_code=category_code)
            ...
            if log is not None:
                ...
                log(
                    f"  batch {attempts}/{max_attempts}: accepted {accepted} "
                    f"of {len(parsed_rows)} parsed rows; total {len(category_rows)}/{rows_per_category}"
                )
        if len(category_rows) < rows_per_category:
            raise RuntimeError(
                f"Generated {len(category_rows)} valid rows for {category_code}; expected {rows_per_category}."
            )
        if log is not None:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    generate_parser = subparsers.add_parser("generate", help="Generate draft labelled rows with local Ollama.")
    generate_parser.add_argument("--rows-per-category", type=int, default=DEFAULT_ROWS_PER_CATEGORY)
    generate_parser.add_argument("--batch-size", type=int, default=DEFAULT_BATCH_SIZE)
    generate_parser.add_argument("--max-retries", type=int, default=DEFAULT_MAX_RETRIES)
